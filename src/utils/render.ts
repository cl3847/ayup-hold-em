...
export async function renderBoard(cards: Card[]): Promise<Buffer> {
    ...
    try {
        ...
        const backgroundImage = await loadImage("assets/table_bottom.png");
        ctx.drawImage(backgroundImage, 0, 0);

        await canvasDrawCommunityCards(ctx, cards);
...
export async function renderUserHand(hand: UserHand) {
    // Create a canvas with width 400 and height 200
    const canvas = createCanvas(1200, 810);
    const ctx = canvas.getContext('2d');
    ...
    try {
        // Load the background image
        const backgroundImage = await loadImage("assets/table_full.png");
        ctx.drawImage(backgroundImage, 0, 0);

        await canvasDrawCommunityCards(ctx, await hand.getShowableCommunityCards());
        await canvasDrawHoleCards(ctx, hand.getHoleCards());
...
async function canvasDrawHoleCards(ctx: CanvasRenderingContext2D, cards: Card[]) {
    const OFFSET_FIRST_CARD_X = 97; // X offset for the first card
    const OFFSET_CARDS_Y = 136; // Y offset for the first card
    const OFFSET_BETWEEN_CARDS_X = 150; // X offset for each subsequent
    const CARD_WIDTH = 119; // Width of each card
    const CARD_HEIGHT = 191; // Height of each card

    const cardImagePromises = cards
        .slice(0, 2) // Take the first 5 cards
        .map(card => {
...
/**
 * Draws up to 5 community cards on an existing canvas.
 * @param ctx - The canvas rendering context to draw on.
 * @param cards - An array of Card objects representing the community cards.
 */
async function canvasDrawCommunityCards(ctx: CanvasRenderingContext2D, cards: Card[]) {
